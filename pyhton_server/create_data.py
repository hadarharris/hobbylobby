...
def get_data():
    ...
    surnames = data["surname"].dropna() # dropna() drops all rows with null values.
    cities_regions = data[["city", "region"]].dropna()
    hobbies = data["hobby"].dropna()
...
def create_data():
    ...
    for user_num in range(number_of_users):
        try:
            ...
            city =  city_and_region['city'].iloc[0]
            region =  city_and_region['region'].iloc[0]
            ...
            cur_user = pd.DataFrame(
                {"user_num":[main.USER_NUM],"name": [name], "gender": [gender], "surname": [surname], "date_of_birth": [date_of_birth],
                 "age": [age], "city": [city], "region": [region], "hobby": [hobby], "email": [email_address], "password": [user_password]})
    ...
    #print(users_df)
    #print(cur_user.head())
    print(users_credentials_array)
    mssql.save_in_sqldb(users_credentials_array)
    # print(len(users))
    users_df.to_csv('newdata.csv', encoding='utf-8', index = False)
    return users_df
